...
try:
    import sys
    sys.path.insert(0, '/GPFS/data/gjliu/Auto-driving/Cop3')
except:
    pass
...
# from team_code.cop3_baseline_infer import CoP3_infer
from team_code.cop3_baseline_infer_lidar import CoP3_infer
from srunner.scenariomanager.carla_data_provider import CarlaDataProvider

from leaderboard.sensors.fixed_sensors import RoadSideUnit, get_rsu_point

def get_entry_point():
    return "CoP3BaselineAgent"
...
#### agents
class CoP3BaselineAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, ego_vehicles_num):
        ...
        ############
        if isinstance(self.config.model, list): # false
            self.ensemble = True
        ...
        ############
        self.infer = CoP3_infer(config=self.config,
    ...
    def spawn_rsu(self):
        ...
        if self.step % self.change_rsu_frame == 0:
            ...
            for vehicle_num in range(self.ego_vehicles_num):
                ...
                # spawn rsu
                self.rsu.append(RoadSideUnit(save_path="/GPFS/data/gjliu/Auto-driving/Cop3/results/rsu_trash",
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.config.fusion_mode == 'none':
            control_all = self.infer.get_action_from_list_for_interfuser(  car_data_raw=ego_data,
                                                            rsu_data_raw=rsu_data, 
                                                            model=self.net, 
                                                            step=self.step,
                                                            timestamp=timestamp)
        elif self.config.fusion_mode == 'inter':
            control_all = self.infer.get_action_from_list(  car_data_raw=ego_data,
                                                            rsu_data_raw=rsu_data, 
                                                            model=self.net, 
                                                            step=self.step,
                                                            timestamp=timestamp)
        elif self.config.fusion_mode == 'cheat':
