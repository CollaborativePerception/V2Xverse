...
from team_code.box_process import boxes_to_corners_3d, get_points_in_rotated_box_3d
...
from team_code.tracker import Tracker, Tracker_lgj
from team_code.interfuser_controller import InterfuserController
from team_code.interfuser_controller_cop3 import InterfuserController_cop3
...
def vis_filter_single(actors_data, view_list):
	...
	actors_data_old = copy.deepcopy(actors_data_filted)
	...
	for view in view_list:
		...
		for N in range(boxes_corner.shape[0]):
			...
			if len(num_lidar_points)<8:
				if id_map[N] in actors_data_filted.keys():
					del actors_data_filted[id_map[N]]

	return actors_data_old, actors_data_filted
    ...
    for i in range(vertical):  # 50
        for j in range(horizontal):  # 25
            if traffic_heatmap[i][j] < 0.05 * 255.0:
                continue
            center_x, center_y = convert_grid_to_xy_lgj(i, j, det_range)
            min_dis = 1000
            min_id = None
            for _id in actors_data:
...
def generate_heatmap(measurements, actors_data, max_distance=18, pixels_per_meter=5):
	img_size = max_distance * pixels_per_meter * 2
...
class DisplayInterface(object):
	...
	def run_interface(self, input_data):
		...
		surface = np.zeros((600, 1800, 3),np.uint8)
		surface[:, :800] = rgb
...
class CoP3_infer():
	def __init__(self, config=None, ego_vehicles_num=1) -> None:
		...
		self.controller = [InterfuserController(self.config) for _ in range(self.ego_vehicles_num)]
	...
	def get_action_from_list(self, car_data_raw, rsu_data_raw, model, step, timestamp):
		...
		control_all = self.generate_action_from_model_output(model_output, car_data_raw, car_data, car_mask, step, timestamp)
		return control_all
	...
	def get_action_from_list_for_interfuser_cheat(self, car_data_raw, rsu_data_raw, model, step, timestamp):
		...
		if self.config.visible_mode == 'single':
			...
			cheat_actor_data_old, cheat_actor_data = vis_filter_single(cheat_actor_data, view_list)
		elif self.config.visible_mode == 'single+rsu':
			view_list = car_data_raw + rsu_data_raw
			cheat_actor_data_old, cheat_actor_data = vis_filter_single(cheat_actor_data, view_list)
			
		for i in range(len(car_data_raw)):
			traffic_meta = generate_heatmap(copy.deepcopy(car_data_raw[i]['measurements']), copy.deepcopy(cheat_actor_data), max_distance=max(self.config.det_range))
			det_data = (
				generate_det_data_lgj(
					traffic_meta, copy.deepcopy(car_data_raw[i]['measurements']), copy.deepcopy(cheat_actor_data), self.config.det_range
				)
                .reshape(int((self.config.det_range[0]+self.config.det_range[1])/self.config.det_range[4]
                            *(self.config.det_range[2]+self.config.det_range[3])/self.config.det_range[4]), -1) #(800,7)
				.astype(np.float32)
			)
	...
	def generate_action_from_model_output(self, model_output, car_data_raw, batch_data, car_mask, step, timestamp):
		(
			traffic_meta_total,
			pred_waypoints_total,
			is_junction_total,
		...
		for count_i in range(self.ego_vehicles_num):
			...
			traffic_meta = traffic_meta_total[ego_i].detach().cpu().numpy() # 400, 7
			pred_waypoints = pred_waypoints_total[ego_i].detach().cpu().numpy() # (10,2)
			...
			if step % 2 == 0 or step < 4:
				...
				traffic_meta = self.trackers[ego_i].update_and_predict(traffic_meta.reshape(int((self.config.det_range[0]+self.config.det_range[1])/self.config.det_range[4]),\
																		 					int((self.config.det_range[2]+self.config.det_range[3])/self.config.det_range[4]), -1), 
							   [car_data_raw[ego_i]['measurements']['gps_x'], car_data_raw[ego_i]['measurements']['gps_y']],  
							   car_data_raw[ego_i]['measurements']['compass'], 
							   step // 2)  # , det_range=self.config.det_range
				traffic_meta = traffic_meta.reshape(int((self.config.det_range[0]+self.config.det_range[1])/self.config.det_range[4]
                            *(self.config.det_range[2]+self.config.det_range[3])/self.config.det_range[4]), -1) #(800,7)
			...
			surround_map, box_info = render(traffic_meta.reshape(int((self.config.det_range[0]+self.config.det_range[1])/self.config.det_range[4]),\
																int((self.config.det_range[2]+self.config.det_range[3])/self.config.det_range[4]), -1), pixels_per_meter=20)
			...
			tick_data[ego_i]["lidar"] = (np.transpose(batch_data[ego_i]["lidar"], (1, 2, 0))*127).astype(np.uint8)
