import numpy as np
import math
from team_code.render import find_peak_box, find_peak_box_lgj
...
class Tracker_lgj():
    ...
    def convert_grid_to_xy(self, i, j, det_range):
        x = det_range[4]*(j + 0.5) - det_range[2]
        y = det_range[0] - det_range[4]*(i+0.5)
        return x, y
    
    def update_and_predict(self, det_data, pos, theta, step, merge_precent=0.4, det_range=[36,12,12,12, 0.25]):
        det_data = det_data * reweight_array
        box_ids, box_info = find_peak_box_lgj(det_data, det_range)
        objects_info = []
        R = np.array(
        ...
        for poi in box_ids:
            i, j = poi
            center_y, center_x = self.convert_grid_to_xy(i, j, det_range) # from pixel coordinate to car coordinate
            center_x = center_x + det_data[i,j,1]
            center_y = center_y + det_data[i,j,2]

            loc = R.T.dot(np.array([center_x, center_y]))
            objects_info.append([loc[0]+pos[0], loc[1]+pos[1], det_data[poi][1:]]) # to world coordinate.
...
class Tracker():
    ...
    def update_and_predict(self, det_data, pos, theta, step, merge_precent=0.4):
        ...
        for poi in box_ids:
            i, j = poi
            center_y, center_x = self.convert_grid_to_xy(i, j) # from pixel coordinate to car coordinate
            center_x = center_x + det_data[i,j,1]
            center_y = center_y + det_data[i,j,2]
