...
def get_max_safe_distance(meta_data, downsampled_waypoints, t, collision_buffer, threshold):
    ...
    return 1000  # safe_distance

class InterfuserController_cop3(object):
    ...
    def run_step(
        ...
        brake = False
        # get desired speed
        downsampled_waypoints = downsample_waypoints(waypoints)  # len:73
        d_0 = get_max_safe_distance(
            meta_data,
            downsampled_waypoints,
            t=0,
            collision_buffer=self.collision_buffer, # array([2.5, 1.2])
            threshold=self.detect_threshold, # 0.04
        )
