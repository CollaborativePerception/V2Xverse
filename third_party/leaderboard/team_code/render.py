...
def find_peak_box_lgj(data, det_range):
...
def render_lgj(det_data, pixels_per_meter=5, max_distance=18, t=0, args=None):
    det_range = args.det_range
    max_distance = max(det_range)
    det_data = det_data * reweight_array
    box_ids = find_peak_box_lgj(det_data, det_range)
    img_size = max_distance * pixels_per_meter * 2
    img = np.zeros((img_size, img_size, 3), np.uint8)
    for poi in box_ids:
        i, j = poi
