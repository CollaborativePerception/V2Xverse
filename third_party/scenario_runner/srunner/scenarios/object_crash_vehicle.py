...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
                 debug_mode=False, criteria_enable=True, adversary_type=False, timeout=60, trigger_distance=10):
        """
        Setup all relevant parameters and create scenario
        """
        self._wmap = CarlaDataProvider.get_map()
    ...
    def _spawn_adversary(self, transform, orientation_yaw):
        ...
        if True:
            ...
            self._other_actor_target_velocity = 3 + (0.4 * self._num_lane_changes) #+ 3
    ...
    def _spawn_blocker(self, transform, orientation_yaw):
        ...
        static = CarlaDataProvider.request_new_actor('static.prop.kiosk_01', self.transform2) # 'static.prop.kiosk_01' 'static.prop.vendingmachine', 'static.prop.busstop'
    ...
    def _spawn_blocker_vehicle(self, transform):
        ...
        if self._reference_waypoint.is_junction:
            ...
            blocker_vehicle_list = ["vehicle.volkswagen.t2","vehicle.carlamotors.carlacola","vehicle.volkswagen.t2"]
            actor_type = random.choice(blocker_vehicle_list)
        static = CarlaDataProvider.request_new_vehicle_actor(actor_type, self.transform2)
        self.vehilce_length = max(static.bounding_box.extent.x,static.bounding_box.extent.y)
        # print(static)
        ...
        while True:  # We keep trying to spawn avoiding props
            ...
            try:
                if vehicle_waypoint:#vehicle_waypoint:  False
                    ...
                    vehicle_location, _ = get_location_in_distance_from_wp(vehicle_waypoint, _start_distance, stop_at_junction=True)
                    ...
                    blocker = self._spawn_blocker_vehicle(waypoint_for_vehicle.transform)
                    self.transform, orientation_yaw = self._calculate_base_vehicle_transform(_start_distance  + self.vehilce_length + 1, waypoint)
                    first_vehicle = self._spawn_adversary(self.transform, orientation_yaw)
                else:
                    # print("spawn walker road blocker")
                    self.trig_distance = self.trigger_distance # 10
                    self.transform, orientation_yaw = self._calculate_base_transform(_start_distance+2, waypoint, k=1.4+bias)
                    first_vehicle = self._spawn_adversary(self.transform, orientation_yaw)
    ...
    def _create_behavior(self):
        ...
        dist_to_trigger = self.trig_distance # + self._num_lane_changes  ## 12 + self._num_lane_changes  -> 12
        # leaf nodes
        ...
        actor_velocity = KeepVelocity(self.other_actors[0],
                                      6.5,  ## self._other_actor_target_velocity  8
