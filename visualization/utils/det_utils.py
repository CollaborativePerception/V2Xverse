...
def generate_det_data(
    heatmap, measurements, actors_data, pixels_per_meter=5, max_distance=48 #18
):
    traffic_heatmap = block_reduce(heatmap, block_size=(10, 10), func=np.mean) #(5, 5)
    traffic_heatmap = np.clip(traffic_heatmap, 0.0, 255.0)
    traffic_heatmap = traffic_heatmap[6:26, 6:26] #[:20, 8:28]
