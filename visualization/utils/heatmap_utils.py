...
def add_circle(img, loc, pixels_per_meter, max_distance, color, radius=5 ):

    center = (loc+max_distance) * pixels_per_meter
    center = np.around(center).astype(np.int)
    color = [int(x) for x in 255 * color]
    cv2.circle(img, tuple(center), radius, color, thickness=-1)
...
def generate_future_waypoints(measurements, pixels_per_meter=5, max_distance=48):
    img_size = max_distance * pixels_per_meter * 2
...
def generate_heatmap(measurements, actors_data, pixels_per_meter=5, max_distance=32):
    img_size = max_distance * pixels_per_meter * 2
    ...
    for i in range(len(measurements['future_waypoints'])+1):

        if i == len(measurements['future_waypoints']):
            loc_gps = [measurements['x_command'],measurements['y_command']]
            color = np.array([1, 0, 0])
            r = 8
        else:
            loc_gps = measurements['future_waypoints'][i][:2]
            color = np.array([0, 1, 0])
            r = 2
        loc = np.zeros(2)
        loc[0] = loc_gps[1]
        loc[1] = -loc_gps[0]
        new_point = R.T.dot(np.array([loc[0] - ego_x, loc[1] - ego_y]))
        waypoint_img = np.zeros((img_size, img_size, 3), np.uint8)
        waypoint_img = add_circle(
            waypoint_img,
            new_point,
            pixels_per_meter,
            max_distance,
        ...
        waypoint_img = np.clip(waypoint_img, 0, 255)
        img = img + waypoint_img      
